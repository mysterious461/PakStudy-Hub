...
import { BookOpenCheck, Building2, CheckCircle2, HelpCircle, Mail, Map, Rocket, Search, Send, ShieldCheck, Users } from "lucide-react";
  ...
  return (
    <PageShell eyebrow="About PakStudy Hub" title="A student-powered academic resource library for Pakistan." text="PakStudy Hub helps students share, discover, and review high-quality academic resources across universities, departments, and courses.">
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <InfoCard icon={Users} title="Our Story" text="Built from the simple student problem of scattered notes, past papers, and course files." />
      </div>
      <div className="mt-6 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <Card className="border-border/60 shadow-sm"><CardContent className="p-6"><h2 className="text-xl font-black">Why PakStudy Hub</h2><p className="mt-3 leading-8 text-muted-foreground">Students should not lose hours searching old group chats, broken drive links, or random folders. PakStudy Hub creates a reviewed, searchable, structured resource library that rewards contributors and protects quality.</p></CardContent></Card>
        <Card className="border-border/60 shadow-sm"><CardContent className="p-6"><h2 className="text-xl font-black">Roadmap</h2><ul className="mt-3 space-y-2 text-muted-foreground"><li>Structured academic taxonomy</li><li>AI summaries, flashcards, and quizzes</li><li>OCR and duplicate detection</li><li>University ambassador network</li></ul></CardContent></Card>
      </div>
  ...
  return (
    <PageShell eyebrow="Contributor Guidelines" title="Upload useful, legal, and well-described study resources." text="These rules keep PakStudy Hub trustworthy for students, contributors, and universities.">
      <div className="grid gap-5 lg:grid-cols-2">
        <InfoCard icon={CheckCircle2} title="Upload Rules" text="Use clear titles, complete hierarchy fields, accurate tags, and upload only material you own or have permission to share." />
        ...
        <InfoCard icon={ShieldCheck} title="Review Process" text="Uploads start pending. Moderators approve, reject, or request changes before public publication." />
        <InfoCard icon={Users} title="Reputation System" text="Approved uploads and helpful contributions increase contributor reputation and badge status." />
      </div>
      <Card className="mt-5 border-border/60 shadow-sm"><CardContent className="p-6"><h2 className="text-xl font-black">Copyright Policy</h2><p className="mt-3 leading-8 text-muted-foreground">Do not upload paid books, private instructor material, copyrighted slides, or restricted documents unless you have explicit permission. Reported resources may be removed and accounts may lose contribution access.</p></CardContent></Card>
    </PageShell>
  );
}
...
export function HelpCenter() {
  ...
  const faqs = ["How do I upload a resource?", "Why is my upload pending?", "What file types are accepted?", "How do I improve my reputation?", "What if my file is rejected?"];
  ...
  return (
    <PageShell eyebrow="Help Center" title="Find answers and get contributor support." text="Search common questions or send a support request for upload and account issues.">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_420px]">
        <div className="space-y-3">{filtered.map((item) => <Card key={item} className="border-border/60 shadow-sm"><CardContent className="p-5"><h3 className="font-black">{item}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">Check the contributor portal, complete required metadata, and follow the review guidelines. More detailed help content will be expanded before public launch.</p></CardContent></Card>)}</div>
        <SupportForm />
      </div>
    </PageShell>
  );
...
function SupportForm() {
  ...
  return (
    <Card className="border-border/60 shadow-sm"><CardContent className="space-y-4 p-6"><h2 className="text-xl font-black">Support request</h2><Input placeholder="Your email" /><Input placeholder="Subject" /><Textarea className="min-h-32 resize-none" placeholder="Tell us how we can help." /><Button className="rounded-2xl font-bold" onClick={() => toast({ title: "Request noted", description: "Support form wiring will connect to backend support tickets next." })}><Send className="mr-2 h-4 w-4" />Send Request</Button></CardContent></Card>
  );
}
